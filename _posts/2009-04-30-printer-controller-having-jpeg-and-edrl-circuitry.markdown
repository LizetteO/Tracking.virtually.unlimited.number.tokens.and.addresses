...
title: Printer controller having JPEG and EDRL circuitry
abstract: A printer controller is provided having page data input circuitry for receiving compressed and encoded page data for expansion processing, JPEG decoding circuitry for decompressing contone data in the received page data, EDRL expansion circuitry for decompressing EDRL-compressed bi-level image data in the received page data, halftoning and compositing circuitry for halftoning and compositing the decompressed contone and bi-level data, and a printhead interface for receiving the compositing data and for writing corresponding print data to a printhead of the printer.
url: http://patft.uspto.gov/netacgi/nph-Parser?Sect1=PTO2&Sect2=HITOFF&p=1&u=%2Fnetahtml%2FPTO%2Fsearch-adv.htm&r=1&f=G&l=50&d=PALL&S1=07876466&OS=07876466&RS=07876466
...
---
This application is a continuation application of application Ser. No. 10 990 382 filed Nov. 18 2004 which is a continuation of Ser. No. 10 636 212 filed Aug. 8 2003 now issued as U.S. Pat. No. 6 851 782 which is a divisional application of application Ser. No. 09 436 747 filed on Nov. 9 1999 issued as U.S. Pat. No. 6 687 022 all of which are herein incorporated by reference.
...
According to the invention there is provided a printer controller for a MEMS based pagewidth inkjet printer the printer controller comprising

memory circuitry for storing compressed and encoded page data to be written to the central processor circuitry.

The decoding circuitry may include JPEG decoding circuitry that is configured to perform a contone page decompression process on the encoded page data EDRL expansion circuitry to decompress EDRL compressed bi level images and halftoning and compositing circuitry to halftone contone data and composite bi level data.

The EDRL expansion circuitry may be configured to generate sets of bi level image lines that are scaled horizontally from a resolution of an expanded bi level image by a suitable scale factor to a print resolution.

The JPEG decoding circuitry may be configured to receive an input in the form of a JPEG bitstream and to generate an output in the form of a set of contone image lines.

The printer controller may include DMA controller circuitry connected to the bus for controlling data transfer between the components of the printer controller.

The printer controller may include quality assurance circuitry the central processor circuitry including an interface for provided communication between the bus and the quality assurance circuitry.

In one broad form the invention comprises in a printer having a printhead with nozzles which discharge a dot of ink by means of a firing pulse generated by an actuator and a printhead interface which transfers data to the print head and controls the nozzle firing sequence during printing the method of controlling the image printed by said printhead comprising the steps of 

an actuator containing a heater for producing firing pulses to discharge ink as dots from said nozzles 

a printhead interface for transferring data to the printhead and for controlling the nozzle firing sequence during printing 

a temperature sensor for sensing the temperature of the printhead and generating a temperature signal 

a voltage sensor for sensing the voltage available to the actuator and generating an available voltage signal 

a programmable pulse duration table located within said printhead interface which receives said temperature and available voltage signals and the viscosity time data from said memory unit and thus generates an output signal and
...
The edge delta and run length EDRL compression format is based loosely on the Group 4 compression format and its precursors 1 18 .
...
A single4 inch printhead contains a total of 25 600 nozzles. A Print Cycle involves the firing of up to all of these nozzles dependent on the information to be printed. A Load Cycle involves the loading up of the printhead with the information to be printed during the subsequent Print Cycle.
...
Each segment has 4 inputs directly related to the cyan magenta yellow and black shift registers. These inputs are called CDataIn MDataIn YDataIln and KDataIn. Since there are 8 segments there are a total of 32 color input lines per 4 inch printhead. A single pulse on the SRClock line shared between all 8 segments transfers the 32 bits into the appropriate shift registers. Alternate pulses transfer bits to the lower and upper nozzles respectively. Since there are 25 600 nozzles a total of 800 pulses are required for the transfer. Once all 25 600 bits have been transferred a single pulse on the shared PTransfer line causes the parallel transfer of data from the shift registers to the appropriate NozzleEnable bits.
...
The state machine in turn outputs the length of the code. This is added modulo 8 to the current codeword bit offset to yield the next codeword bit offset. The bit offset in turn controls the barrel shifter . If the codeword bit offset wraps then the carry bit controls the latching of the next byte from the input FIFO . At this time byte is latched to byte and the FIFO output is latched to byte . It takes two cycles of length 8 to fill the input buffer. This is handled by starting states in the state machine .
